    ...
    <header
      className={`header left-0 top-0 z-40 flex w-full items-center ${
        sticky
          ...
          : "absolute z-[9999] bg-white bg-opacity-95 shadow-sticky backdrop-blur-sm transition"
      }`}
    >
        ...
        <div className="relative -mx-4 flex items-center justify-between">
          ...
          <div className="flex w-full items-center justify-end px-4">
            <div>
              ...
              >
                <ul className="block lg:flex lg:space-x-12 lg-max:space-x-8">
                  {menuData.map((menuItem, index) => (
                    <li key={index} className="group relative">
                      {menuItem.path ? (
                        <Link
                          href={menuItem.path}
                          className={`flex py-2 text-lg lg:mr-0 font-medium lg:inline-flex lg:px-0 lg:py-6 ${
                        ...
                        <>
                          ...
                          <div
                            className={`submenu relative left-0 top-full rounded-sm bg-white transition-[top] duration-300 group-hover:opacity-100lg:invisible lg:absolute lg:top-[110%] lg:block lg:w-[250px] lg:p-4 lg:opacity-0 lg:shadow-lg lg:group-hover:visible lg:group-hover:top-full ${
                              openIndex === index ? "block" : "hidden"
                            }`}
                          >
                            {menuItem.submenu &&
                              menuItem.submenu.map(
                                (submenuItem, index: number) => (
                                  <Link
                      ...
                      <div className="flex space-x-2">
                        ...
                        <span className="py-2 text-lg lg:mr-0 font-medium lg:inline-flex lg:px-0 lg:py-6 text-dark">|</span>
                        <Link
                          href={pathname} 
                          locale="nl"
                          className={`flex py-2 text-lg lg:mr-0 font-medium lg:inline-flex lg:px-0 lg:py-6 ${
